...
-   To work with the `ImageLabels` class, we need to convert the labels_data to the required format which is represented as a dictionary of lists of tuples. The dictionary keys are the image keys, and the values are lists of tuples of the form (row, column, label_id).
...
labels_data = {
    ...
    for key, group in appended_df.groupby("key")
}
```
